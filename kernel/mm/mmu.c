...
mmu_init(void)
{
  ...
  mmu_map_static(kern_trtab, KERNEL_BASE, 0, PHYS_TOP, VM_READ | VM_WRITE);

  // Map all devices 
  mmu_map_static(kern_trtab, KERNEL_BASE + PHYS_TOP, PHYS_TOP,
  ...
  mmu_map_static(kern_trtab, VECTORS_BASE, (uint32_t) _start, PAGE_SIZE, VM_READ);
